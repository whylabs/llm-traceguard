    ...
    def build(self) -> dict:
        ...
        if self.custom_properties:
            for key, value in self.custom_properties.items():
                meta[f"WhyLabs-Property-{key}"] = value

        if self.user_id:
            meta["WhyLabs-User-Id"] = self.user_id

        if self.node_id:
            meta["WhyLabs-Node-Id"] = self.node_id

        return meta


class CreateArgsExtractor:
    _traceguard_meta: Optional[RequestMeta]
    ...
    def __init__(self,
        ...
        self._traceguard_meta = kwargs.get("traceguard_meta")
        self.kwargs.pop("traceguard_meta", None)
    ...
    def get_helicone_meta(self) -> dict:
        if (self._traceguard_meta is None):
            return {}
        return self._traceguard_meta.build()
...
class OpenAIInjector:
    ...
    def _with_helicone_auth(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # )
            print("providerRequest", arg_extractor.get_body())
            try:
    ...
    def apply_traceguard(self_parent):
